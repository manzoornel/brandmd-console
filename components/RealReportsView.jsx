...
export default function RealReportsView({ generatedAt, people, videos, attendance, attendanceEvents, activityEvents, logs, clients, compensationRules, sundayCredits }) {
  ...
  const rows = useMemo(() => staff.map((p) => {
    ...
    const published = videos.filter(v => v.writer_id === p.id && v.item_type !== "shoot" && inRange(v.posted_at));
    ...
    return { ...p, role: rolesLabel(p.roles || []), edited, staffUnits, shoots, images, yt, ig, fb, posts: published.length, hours, verifiedClockIns, activeSeconds, turnaround, rework };
  ...
  const events = useMemo(() => (activityEvents || []).filter(e => inRange(e.created_at)).map(e => {
    const v = videos.find(v => v.id === e.video_id) || {};
    const person = people.find(p => p.id === e.user_id)?.full_name || "System";
    const client = clients.find(c => c.id === v.client_id)?.name || "No doctor";
    const m = e.metadata || {};
    const labels = { created:"Created in To-Do", item_saved:"Details saved", content_saved:"Content saved", stage_transition: m.to_stage === "review" ? "Sent to review" : m.to_stage === "content" ? "Review approved" : m.to_stage === "published" ? "Published" : m.to_stage === "to_edit" ? "Returned to To-Do" : "Stage changed" };
    return { at:e.created_at, user:person, action:labels[e.event_type] || e.event_type, title:v.title || "Deleted item", client };
  }).sort((a,b) => new Date(b.at)-new Date(a.at)).slice(0,100), [activityEvents, videos, people, clients, start, end]);
  const legacyEvents = useMemo(() => videos.flatMap(v => {
    const client = clients.find(c => c.id === v.client_id)?.name || "No doctor";
  ...
  return <div className="body">
    ...
    {tab === "Overview" && <><section className={styles.kpis}><Kpi icon="▶" color="violet" value={total("edited")} label="Videos edited"/><Kpi icon="●" color="amber" value={total("shoots")} label="Shoots completed"/><Kpi icon="↗" color="green" value={total("posts")} label="Content posted"/><Kpi icon="◇" color="coral" value={total("images")} label="Posters / images"/></section><section className={styles.grid2}><div className={styles.panel}><Title title="Output by worker" sub="Each content item is counted once, even when shared to several platforms"/>{rows.map(w => <Bar key={w.id} w={w} max={Math.max(1,...rows.map(x=>x.edited+x.shoots+x.images+x.posts))}/>)}</div><div className={styles.panel}><Title title="Platform link coverage" sub="Platform links are shown separately and do not multiply staff work"/><Donut yt={total("yt")} ig={total("ig")} fb={total("fb")}/><div className={styles.legend}><span><i className={styles.yt}/>YouTube <b>{total("yt")}</b></span><span><i className={styles.ig}/>Instagram <b>{total("ig")}</b></span><span><i className={styles.fb}/>Facebook <b>{total("fb")}</b></span></div></div></section>{top && <section className={styles.insight}><span>★</span><div><b>{top.full_name} leads output for this period</b><p>Based on edits, completed shoots, content items and creatives.</p></div><button onClick={()=>setTab("Workers")}>View worker report →</button></section>}</>}
    {tab === "Workers" && <div className={styles.panel}><Title title="Worker performance" sub="Live output, attendance and staff incentive units"/><table><thead><tr><th>Worker</th><th>Role</th><th>Edits</th><th>Staff units</th><th>Shoots</th><th>Content posted</th><th>Images</th><th>Verified days</th><th>Hours</th><th>Rework*</th><th>Avg. turnaround</th></tr></thead><tbody>{rows.map(w=><tr key={w.id}><td><b>{w.full_name}</b></td><td>{w.role}</td><td>{w.edited}</td><td><b>{w.staffUnits.toFixed(1)}</b></td><td>{w.shoots}</td><td>{w.posts}</td><td>{w.images}</td><td>{w.verifiedClockIns}</td><td>{w.hours.toFixed(1)}h</td><td>{w.rework}</td><td>{w.turnaround ? `${w.turnaround.toFixed(1)} days` : "—"}</td></tr>)}</tbody></table><p className="hint">* Each published content item counts once. YouTube, Instagram and Facebook links are tracked separately without multiplying staff output.</p></div>}
    ...
    {tab === "Stage timing" && <div className={styles.panel}><Title title="Workflow turnaround" sub="Only stage completions in the selected calendar period are shown"/><table><thead><tr><th>Item</th><th>Doctor</th><th>Edit → review</th><th>Review → approval</th><th>Content → published</th><th>Published</th></tr></thead><tbody>{videos.filter(v=>inRange(v.submitted_at)||inRange(v.approved_at)||inRange(v.posted_at)).map(v=>{const c=clients.find(c=>c.id===v.client_id)?.name||"—";return <tr key={v.id}><td><b>{v.title}</b></td><td>{c}</td><td>{v.submitted_at?`${hoursBetween(v.created_at,v.submitted_at).toFixed(1)} h`:"—"}</td><td>{v.approved_at&&v.submitted_at?`${hoursBetween(v.submitted_at,v.approved_at).toFixed(1)} h`:"—"}</td><td>{v.posted_at&&v.approved_at?`${hoursBetween(v.approved_at,v.posted_at).toFixed(1)} h`:"—"}</td><td>{v.posted_at&&inRange(v.posted_at)?new Date(v.posted_at).toLocaleString():"—"}</td></tr>})}</tbody></table></div>}
    {tab === "Activity log" && <div className={styles.panel}><Title title="Current-period activity timeline" sub="Each save, review, approval and publication keeps its own timestamp"/><div className={styles.timeline}>{shownEvents.map((e,i)=><div className={styles.event} key={`${e.at}-${i}`}><span className={styles.eventDot}/><time>{new Date(e.at).toLocaleString()}</time><b>{e.user}</b><strong>{e.action}</strong><p>{e.client} · {e.title}</p><em>Recorded</em></div>)}</div></div>}
  </div>;
}
...
function Donut({yt,ig,fb}){const total=Math.max(yt+ig+fb,1),a=yt/total*360,b=(yt+ig)/total*360;return <div className={styles.donut} style={{background:`conic-gradient(#ff5d5d 0 ${a}deg,#6c4ff8 ${a}deg ${b}deg,#2f80ed ${b}deg 360deg)`}}><div><b>{yt+ig+fb}</b><span>Platform links</span></div></div>}
