...
export default function RealReportsView({ generatedAt, people, videos, attendance, attendanceEvents, logs, clients, compensationRules, sundayCredits }) {
  ...
  return <div className="body">
    ...
    {tab === "Live attendance" && <div className={styles.panel}><Title title="Login & worked-time live view" sub="Every login session recorded during the selected period"/><table><thead><tr><th>Staff</th><th>Role</th><th>Date</th><th>Login</th><th>Logout</th><th>Worked</th><th>Status</th></tr></thead><tbody>{attendanceRows.map((a,i)=><tr key={`${a.user_id}-${a.clock_in}-${i}`}><td><b>{a.name}</b></td><td>{a.role}</td><td>{new Date(a.clock_in).toLocaleDateString()}</td><td>{fmtTime(a.clock_in)}</td><td>{a.live ? "—" : fmtTime(a.clock_out)}</td><td><b>{a.worked.toFixed(1)} h</b></td><td>{a.live ? <b style={{color:"#059669"}}>● Live now</b> : "Completed"}</td></tr>)}{!attendanceRows.length&&<tr><td colSpan="7">No attendance sessions in this period.</td></tr>}</tbody></table></div>}
