...
create table if not exists videos (
  id uuid primary key default gen_random_uuid(),
  title text not null,
  item_type text not null default 'video' check (item_type in ('video','poster','shoot')),
  brief text default '',
  due_date date,
...
returns boolean language sql stable security definer set search_path = public as $$
  select has_role('super_admin') or has_role('admin')
      or has_role('editor') or has_role('writer') or has_role('designer') or has_role('shooter')
$$;
...
-- ---------- v3 r3c: shooting role insert + firm hierarchy reads ----------
...
-- ---------- v3 r3d: expenses / assets / partners (accounting) ----------
