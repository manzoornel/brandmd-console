...
export const effectiveVideoUnits = (video) => {
  ...
  if (video?.calculated_units !== null && video?.calculated_units !== undefined) {
    const stored = Math.max(0, Number(video.calculated_units) || 0);
    if (stored > 0 || video?.item_type !== "video") return stored;
  }
  const calculated = videoUnitsFromSeconds(video?.duration_seconds);
  // Videos created before duration tracking remain one legacy unit.
  return calculated || (video?.item_type === "video" ? 1 : 0);
};
...
export const effectiveStaffVideoUnits = (video) => {
  ...
  if (video?.calculated_staff_units !== null && video?.calculated_staff_units !== undefined) {
    const stored = Math.max(0, Number(video.calculated_staff_units) || 0);
    if (stored > 0 || video?.item_type !== "video") return stored;
  }
  const calculated = staffVideoUnitsFromSeconds(video?.duration_seconds);
  // Preserve credit for completed legacy edits that have no saved duration.
  return calculated || (video?.item_type === "video" ? 1 : 0);
};
...
export function compensationBreakdown(rule, metrics) {
  ...
  return {
    ...
    warnings: [
      videoUnits < (Number(rule?.monthly_video_unit_target) || 0) ? `${((Number(rule?.monthly_video_unit_target) || 0) - videoUnits).toFixed(1)} video unit target short` : null,
