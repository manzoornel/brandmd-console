...
export default async function AccountsPage() {
  ...
  const [{ data: clients }, { data: payments }] = await Promise.all([
    ...
    supabase.from("payments").select("client_id, amount, paid_at, note").order("paid_at", { ascending: false }),
  ]);
  ...
  return (
    <div className="body">
      ...
      <table className="tbl" style={{ marginTop: 18 }}>
        <thead><tr><th>Doctor</th><th>Package</th><th>Total</th><th>Collected</th><th>Remaining</th><th>Follow-up</th><th></th></tr></thead>
        <tbody>
          {billed.map((c) => {
          ...
          {billed.length === 0 && <tr><td colSpan="7" style={{ color: "#9aa1b3" }}>No paid packages yet. Set a price on a doctor under Doctors.</td></tr>}
        </tbody>
      </table>
    </div>
  );
